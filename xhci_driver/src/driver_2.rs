...
use crate::{
    ...
    doorbell::{DoorbellArray, DoorbellRegister, HostControllerDoorbellTarget},
    event_ring::XhciErstEntry,
    interrupter_regs::InterrupterRegsVolatileFieldAccess,
    mem::{
        XHCI_COMMAND_RING_SEGMENTS_ALIGNMENT, XHCI_COMMAND_RING_SEGMENTS_BOUNDARY,
        XHCI_DEVICE_CONTEXT_ALIGNMENT, XHCI_DEVICE_CONTEXT_BOUNDARY,
        XHCI_EVENT_RING_SEGMENT_TABLE_ALIGNMENT, XHCI_EVENT_RING_SEGMENT_TABLE_BOUNDARY,
        XHCI_EVENT_RING_SEGMENTS_ALIGNMENT, XHCI_EVENT_RING_SEGMENTS_BOUNDARY,
    },
    operational_regs::{OperationalRegs, OperationalRegsVolatileFieldAccess},
    runtime_regs::{RuntimeRegisters, RuntimeRegistersVolatileFieldAccess},
    trb::{TransferRequestBlock, TrbControl},
    trb_type::XhciTrbType,
};

pub unsafe fn start_2(bar0: NonZero<usize>, allocate: impl Fn(AllocRequest) -> AllocResponse) {
    let capability_regs = {
    ...
    *command_ring.last_mut().unwrap() = TransferRequestBlock {
    ...
    operational_regs.as_mut_ptr().crcr().update(|mut crcr| {
        ...
        crcr.set_ring_cycle_state(producer_cycle_state);
        crcr
    });
    ...
    let event_ring_segment_table_size = event_ring_segment_table_len * size_of::<XhciErstEntry>();
    ...
    // Send a command
    // TODO: Properly enqueue
    command_ring[0] = TransferRequestBlock {
        parameter: 0,
        status: 0,
        control: {
            // let mut control = TrbControl(0);
