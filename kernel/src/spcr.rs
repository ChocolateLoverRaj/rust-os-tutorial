    ...
    for<'a> OffsetPageTable<'a>: Mapper<S>,
{
    if let Some(uart) = acpi_tables
        .find_table::<Spcr>()
        .inspect(|result| log::debug!("SPCR: {:?}", result.deref()))
        .ok()
        .and_then(|spcr| match spcr.interface_type() {
            SpcrInterfaceType::Full16550
            | SpcrInterfaceType::Full16450
            | SpcrInterfaceType::Generic16550 => spcr.base_address(),
            _ => None,
        })
        .inspect(|_| log::debug!("Valid interface type"))
        .and_then(|base_address| base_address.ok())
        .inspect(|_| log::debug!("Valid base address"))
        ...
        .filter(|base_address| match base_address.address_space {
            AddressSpace::SystemMemory => true,
            _ => false,
        })
        .inspect(|_| log::debug!("Base address is MMIO"))
        .filter(|base_address| {
            base_address.bit_offset == 0 && base_address.bit_width.is_multiple_of(8)
        })
        .inspect(|_| log::debug!("Valid bit offset and bit width"))
        .map(|base_address| {
            ...
            log::debug!(
                "Creating memory mapped uart writer at {base_pointer:?} with stride {stride_bytes}"
            );
        ...
        })
    {
        ...
        log::info!("Replaced COM1 UART with UART specified by SPCR");
    }
}
