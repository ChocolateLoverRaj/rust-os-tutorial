use common::{SliceData, SyscallAlloc2, SyscallAlloc2Error};
...
pub struct SyscallAlloc2Handler;
impl GenericSyscallHandler for SyscallAlloc2Handler {
    type S = SyscallAlloc2;
        ...
        if align.checked_next_power_of_two() == Some(align) {
            helper.syscall_return(&(|| {
                ...
                let range = task
                    ...
                    .ok_or(SyscallAlloc2Error::OutOfVirtualMemory)?;
                ...
                for page in start_page..=end_page_inclusive {
                    let frame = physical_memory
                        ...
                        .ok_or(SyscallAlloc2Error::OutOfPhysicalMemory)?;
                    // Zero the frame
                    ...
                    unsafe { mapper.map_to(page, frame, flags, frame_allocator) }
                        .map_err(|e| match e {
                            MapToError::FrameAllocationFailed => {
                                SyscallAlloc2Error::OutOfPhysicalMemory
                            }
                            e => unreachable!("{:#?}", e),
                        })?
                        .flush();
