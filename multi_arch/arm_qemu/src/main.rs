...
extern "C" fn _start() {
    naked_asm!(
        ...
        b start
        .space 32
        .word 0x016f2818      @ Magic
        .word __kernel_start   @ Start
        .word __kernel_end     @ End (Calculated by Linker)
...
extern "C" fn kernel_main_32(_r0: usize, _machine_id: usize, atags_fdt_ptr: usize) -> ! {
    logger::init();

    info!("ATAGS / DeviceTree ptr: {atags_fdt_ptr:#X}");
    let result = unsafe { Fdt::from_ptr(atags_fdt_ptr as *mut _) };
    info!("FDT: {result:#?}");
