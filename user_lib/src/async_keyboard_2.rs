...
use alloc::boxed::Box;
use common::{QueueReader, SyscallSubscribeToKeyboard2, log};
use futures::Stream;

use crate::{ExecutorContext, syscall};
...
impl<'a> AsyncKeyboard2<'a> {
    ...
    pub fn new(executor_context: &'a ExecutorContext, slots_len: usize) -> Self {
        ...
        log::debug!("write addr {:p}", &mem.write_count);
        let input = b.as_mut_ptr() as u64;
        // Safety: The pointer points to valid memory
        let event_id = unsafe { syscall::<SyscallSubscribeToKeyboard2>(&input) };
        Self {
            executor_context,
            event_id,
            b,
...
impl Drop for AsyncKeyboard2<'_> {
    fn drop(&mut self) {
...
impl Stream for AsyncKeyboard2<'_> {
    type Item = u8;
